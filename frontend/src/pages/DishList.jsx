...
export default function DishList() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const items = await api.getDishs();
      setData(items);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (e) => {
      ...
      if (msg.channel === 'dataChange' && msg.data.model === 'Dish') fetchData();
    };
    return () => ws.close();
  }, []);
  ...
  const handleDelete = async (id) => {
    try { await api.deleteDish(id); message.success('删除成功'); fetchData(); }
  ...
  const handleOk = async () => {
    ...
    try {
      if (editingItem) {
        await api.updateDish(editingItem.id, values);
        message.success('更新成功');
      } else {
        await api.createDish(values);
