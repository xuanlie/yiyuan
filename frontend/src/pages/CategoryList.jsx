...
export default function CategoryList() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const items = await api.getCategorys();
      setData(items);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (e) => {
      ...
      if (msg.channel === 'dataChange' && msg.data.model === 'Category') fetchData();
    };
    return () => ws.close();
  }, []);
  ...
  const handleDelete = async (id) => {
    try { await api.deleteCategory(id); message.success('删除成功'); fetchData(); }
  ...
  const handleOk = async () => {
    ...
    try {
      if (editingItem) {
        await api.updateCategory(editingItem.id, values);
