...
export default function TableList() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const items = await api.getTables();
      setData(items);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (e) => {
      ...
      if (msg.channel === 'dataChange' && msg.data.model === 'Table') fetchData();
    };
    return () => ws.close();
  }, []);
  ...
  const handleDelete = async (id) => {
    try { await api.deleteTable(id); message.success('删除成功'); fetchData(); }
  ...
  const handleOk = async () => {
    ...
    try {
      if (editingItem) {
        await api.updateTable(editingItem.id, values);
